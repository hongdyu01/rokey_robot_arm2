...
class ExtractKeyword:
    def __init__(self):
        ...
        prompt_content = """
            당신은 사용자의 문장에서 특정 과일을 추출해야 합니다.

            <목표>
            - 문장에서 다음 리스트에 포함된 과일의 인덱스를 출력해야함
            ...
            <출력 형식>
            - 다음 형식을 반드시 따르세요: [과일1 과일2]
            - 과일은 공백으로 구분
            - 과일이 없으면 앞쪽은 공백 없이 비웁니다

            <특수 규칙>
            - 명확한 과일 명칭이 없지만 문맥상 유추 가능한 경우(예: "빨갛고 둥근것" → apple)는 리스트 내 항목으로 최대한 추론해 반환하세요.
            <예시>
            - 입력: "apple"  
            출력: 0
    ...
    def extract_keyword(self, output_message):
        ...
        print(f"llm's response(object): {int(object)}")
...
class TestNode(Node):
    ...
    #output_message = "못 밖는데 사용되는 1번 위치 넣어줘"
                ...
                user_input = keyword#input("무엇을 집을까요? (name/number, help, q): ")
            except EOFError:
                self.stop_event.set()
                break
